...
public interface SpringDataJpaSpitterRepository
    extends JpaRepository<SpringDataJpaSpitter, Long>, SpringDataJpaSpitterSweeper {
  ...
  SpringDataJpaSpitter findByUsername(String username);

  List<SpringDataJpaSpitter> findByUsernameOrFullNameLike(String username, String fullName);

  // 自定义查询：不符合Spring Data的方法命名约定，可以用@Query注解来指定sql
  @Query("select s from SpringDataJpaSpitter s where s.email like '%gmail.com' ")
  List<SpringDataJpaSpitter> findAllGmailSpitters();
