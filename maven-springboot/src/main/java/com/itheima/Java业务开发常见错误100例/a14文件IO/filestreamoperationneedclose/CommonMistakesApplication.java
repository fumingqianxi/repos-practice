package com.itheima.Java业务开发常见错误100例.a14文件IO.filestreamoperationneedclose;
...
//  }
  ...
  private static void linesTest() throws IOException {
    ...
    log.info(
        "lines {}",
        Files.lines(Paths.get("large.txt")).limit(200000).collect(Collectors.toList()).size());
    stopWatch.stop();
    stopWatch.start("read 2000000 lines");
    log.info(
        "lines {}",
        Files.lines(Paths.get("large.txt")).limit(2000000).collect(Collectors.toList()).size());
    stopWatch.stop();
